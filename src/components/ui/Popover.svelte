<script lang="ts">
    ...
    let { trigger, children, class: className = "", label = "Open popover", onOpen }: Props = $props();
    ...
    function updatePosition() {
        ...
        left = Math.max(VIEWPORT_PADDING, Math.min(left, vw - VIEWPORT_PADDING - width));
        top = Math.max(VIEWPORT_PADDING, Math.min(top, vh - VIEWPORT_PADDING - height));
